...
function Border({ size = 1, radius = 0, color = hex("#ffffff"), transparency = 0, children }: Props) {
	return (
		<frame
		...
		>
			...
			<uicorner
				CornerRadius={React.joinBindings({ radius: asBinding(radius), size: asBinding(size) }).map(
